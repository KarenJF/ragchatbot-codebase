...
class CourseSearchTool(Tool):
    ...
    def _format_results(self, results: SearchResults) -> str:
        ...
        for doc, meta in zip(results.documents, results.metadata):
            ...
            # Create source entry with embedded link using delimiter format
            if lesson_link:
                # Format as "display_text|url" for frontend parsing
                source_entry = f"{source_text}|{lesson_link}"
            else:
...
class CourseOutlineTool(Tool):
    ...
    def execute(self, course_name: str) -> str:
        ...
        for course in all_courses:
            if course.get('title') == actual_course_title:
                course_data = course
                break
        
        if not course_data:
            return f"Course metadata not found for '{actual_course_title}'"
    ...
    def _format_outline(self, course_data: Dict[str, Any]) -> str:
        ...
        source_entry = course_title
        if course_link:
            source_entry = f"{course_title}|{course_link}"
        
        self.last_sources = [source_entry]
